...
# 	 )
#dirlist=($(ls -d conf_nc4nf1_248_*m0p0[15]00 ))
#dirlist=($(ls -d conf_nc4nf1_248_*m0p0[15]00 ))
dirlist=($(ls -d conf_nc4nf1_248_b10p8[3467]*_m0p0500 conf_nc4nf1_248_b10p8*_m0p0100 conf_nc4nf1_248_b11p*_m0p0[15]00 ))
...
while true; do
	...
	if [ ! -e "$JOB1" ]; then
		...
		cmd="flux batch --queue=pbatch  --job-name=${label}_cont ../fluxauto_cont_248_N8_M51.5.sh > ${JOB0}"
		echo ${cmd}
		eval ${cmd}
	    fi
		...
		# R: running
		cmd="flux batch --queue=pbatch --job-name=${label}_cont --dependency=afterany:$(cat $JOB0) ../fluxauto_cont_248_N8_M51.5.sh > ${JOB1}"
		echo ${cmd}
		eval ${cmd}
