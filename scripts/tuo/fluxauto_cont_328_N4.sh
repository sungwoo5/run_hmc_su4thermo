...
PARAMS=" --grid 32.32.32.8 --mpi 4.2.2.1 --threads 20 --accelerator-threads 8 ${OPTIONS} --ParameterFile ${XML}"
...
# ln -s ../${lastconf} .
# ln -s ../${lastrng} .
# flux run -N 4 --tasks-per-node=4 --verbose --exclusive --setopt=mpibind=verbose:1 bash -c "flux hostlist -le; $APP $PARAMS" &> ${LOG2} &
